...
public class MovieDAO {
    ...
    public static ArrayList<Movie> get_all() {
        return get_for_query("SELECT * FROM movie;");
    }

    public static ArrayList<Movie> get_for_range(int offset, int num_results) {
        return get_for_query(
               String.format("SELECT * FROM movie LIMIT %d, %d", offset, num_results)
        );
    }

    public static int total_movies() {
        try {
            ResultSet rs = Database.get_data_result("SELECT COUNT(*) FROM movie");
            rs.next();
            return rs.getInt(1);
        } catch (Exception ex) {
            ex.printStackTrace();
